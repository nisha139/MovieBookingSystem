...
namespace MovieBooking.Application.Models.Specification;
public static class SpecificationBuilderExtensions
{
        ...
        Filter? filter)
    {
        if (filter is not null)
        {
            ...
            {
                if (filter.Filters is null) throw new InternalServerException("The Filters attribute is required when declaring a logic");
        ...
        Type propertyType)
    {
        ...
        if (propertyType.IsEnum)
        {
            ...
            if (!Enum.TryParse(propertyType, stringEnum, true, out var valueparsed)) throw new CustomException(string.Format("Value {0} is not valid for {1}", value, field));
    ...
    public static dynamic? ChangeType(object value, Type conversion)
    {
        ...
        if (t is not null && t.Name == "DateTime")
        {
