...
public class SmtpMailService : IMailService
{
    ...
    public async Task SendAsync(MailRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // From
            email.From.Add(new MailboxAddress(!string.IsNullOrWhiteSpace(request.DisplayName) ? request.DisplayName : _settings.DisplayName, !string.IsNullOrWhiteSpace(request.From) ? request.From : _settings.From));
